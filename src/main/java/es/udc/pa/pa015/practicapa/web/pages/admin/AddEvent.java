...
public class AddEvent {
	...
	void onPrepareForRender() {
		...
		for (CategoryInfo temp : tempCategories) {
			if (categories == null) {
				categories = (temp.getCategoryId().toString() + "=" + temp.getCategoryName());
			} else {
				categories += (", " + temp.getCategoryId().toString() + "=" + temp.getCategoryName());
			}
		}
	...
	void onValidateFromAddEventForm() throws NullEventNameException {
		...
		try {
			...
			addEventForm.recordError(startDateTextField, messages.format("error-dateHasPassed", startDate));
			addEventForm.recordError(startTimeTextField, messages.format("error-timeHasPassed", startTime));
		} catch (InstanceNotFoundException ex) {
			addEventForm.recordError(categoryIdSelect, messages.format("error-categoryDoesNotExist", categoryId));
